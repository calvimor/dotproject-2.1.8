...
* 
* Adapted to github.com/jean-lelarge/dotproject-2.1.8
* 
 ...
 */
function checkFlagx($flag, $perm_type, $old_flag)
{
    if ($old_flag) {
 ...
 */
function isAllowedx($perm_type, $mod, $item_id = 0)
{
    $invert = false;
    switch ($perm_type) {
        case PERM_READ:
...
function getPermissionx($mod, $perm, $item_id = 0)
{
...
function array_csortx() { //coded by Ichier2003
...
*/
function calcEndByStartAndDurationx($task)
{
...
// from modules/tasks/tasks.class.php
function sort_by_item_titlex($title, $item_name, $item_type, $a = '')
{
...
function showchilddeptx(&$a, $level = 1)
{
    global $department;
...
function findchilddeptx(&$tarr, $parent, $level = 1)
{
    $level++;
    $n = count($tarr);
    for ($x = 0; $x < $n; $x++) {
...
//comes from modules/departments/departments.class.php
function addDeptIdx($dataset, $parent)
{
    global $dept_ids;
...
function last_filex($file_versions, $file_name, $file_project)
{
    $latest = null;
...
function projects_list_datax($user_id = false)
{
...
##
function bestColorx($bg, $lt = '#ffffff', $dk = '#000000')
{
    // cross-over color = x
    $x = 128;
...
##
function arraySelectx(&$arr, $select_name, $select_attribs, $selected, $translate = false)
{
    global $AppUI;
    if (!is_array($arr)) {
...
## returns a select box based on an key,value array where selected is based on key
##
function arraySelectTreex(&$arr, $select_name, $select_attribs, $selected, $translate = false)
{
    reset($arr);
...
function tree_recursex($id, $indent, $list, $children)
{
...
function projectSelectWithOptGroupx($user_id, $select_name, $select_attribs, $selected, $excludeProjWithId = null)
{
    global $AppUI;
    $q = new w2p_Database_Query();
    $q->addTable('projects', 'pr');
...
function w2PfindImage($name, $module = null)
{
    ...
    } elseif ($module && file_exists(W2P_BASE_DIR . '/style/' . dPgetConfig('host_style') . '/images/modules/' . $module . '/' . $name)) {
        return './style/' . dPgetConfig('host_style') . '/images/modules/' . $module . '/' . $name;
    } elseif (file_exists(W2P_BASE_DIR . '/style/' . dPgetConfig('host_style') . '/images/icons/' . $name)) {
        return './style/' . dPgetConfig('host_style') . '/images/icons/' . $name;
    } elseif (file_exists(W2P_BASE_DIR . '/style/' . dPgetConfig('host_style') . '/images/obj/' . $name)) {
        return './style/' . dPgetConfig('host_style') . '/images/obj/' . $name;
 ...
 */
function defValx($var, $def)
{
    return isset($var) ? $var : $def;
}

function addHistoryx($table, $id, $action = 'modify', $description = '', $project_id = 0)
{
    global $AppUI;
    /*
    * TODO:
...
{
    $memoryLimt = (dPgetConfig('reset_memory_limit') != '') ? dPgetConfig('reset_memory_limit') : '64M';
 ...
 */
function formatCurrencyx($number, $format)
{
...
function format_backtracex($bt, $file, $line, $msg)
{
...
function dprintx($file, $line, $level, $msg)
{
 ...
 */
function findTabModulesx($module, $file = null)
{
    $modlist = array();
...
function getUsersArrayx()
{
    return w2PgetUsersHashList();

}

function getUsersCombox($default_user_id = 0, $first_option = 'All users')
{
    global $AppUI;
...
function w2PcheckMem($min = 0, $revert = false)
{
    ...
    $want = w2PgetBytes(dPgetConfig('reset_memory_limit'));
    $have = ini_get('memory_limit');
    // Try upping the memory limit based on our config
    ini_set('memory_limit', dPgetConfig('reset_memory_limit'));
...
function sendNewPass()
{
    ...
    $checkusername = preg_replace("/[^A-Za-z0-9]/", "", dPgetParam($_POST, 'checkusername', ''));
...
function showcompany($company_id, $restricted = false)
{
    ...
    foreach ($projects as $project => $name) {
        ...
        if (($template_status = dPgetConfig('template_projects_status_id')) != '') {
...
// query my sub-tasks (ignoring task parents)
    ...
    if (!$showHoldProjs) {
        if (($on_hold_status = dPgetConfig('on_hold_projects_status_id')) != '') {
...
function __extract_from_contact_controller($controller)
{
    ...
    $notifyasked = dPgetParam($_POST, 'contact_updateask', 0);
...
//TODO: This whole structure is hard-coded based on the TaskStatus SelectList.
    ...
    if ($min_view && isset($_GET['task_status'])) {
        $task_status = (int)dPgetParam($_GET, 'task_status', null);
        return $task_status;
