...
# $Id: newgateway.pl,v 1.41 2006/12/11 kang Exp $ #
...
$report_to_address = 'support@yourcompany.com';

# report from address
$report_from_address = 'support@yourcompany.com';
...
# dummy company and contact id and name
# used for unknown requestor
# you need to create a dummy contact and a dummy company in your dotproject and assign their ids here
$unknown_contactname='Unknown Unknown';
$unknown_contactid=;
$unknown_compid=;

######################## </CONFIGURATION SECTION> ##############################
# Please read the meanings of those configurations and make changes to meet your needs

## First phase, check to see we can configure ourselves based upon
## the dotproject and helpdesk environment.
## Two files are needed as input arguments: the configuration file of dotproject and the configuration file of helpdesk.
die ("Newgateway.pl requires the full path to config.php files of dP and Helpdesk as its arguments") if ($#ARGV != 1);
%config = ();
&check_dPconfig($ARGV[0]);

%HDconfig=();
&check_HDconfig($ARGV[1]);

# Shortcuts for the email code
$app_root = "https://yourcompany.com/dotproject";
...
sub check_HDconfig() {
  $hd_conf = $_[0];
    open (PHPCONFIG, "<$hd_conf")
        or die ("Cannot find helpdesk configuration file!");
    while (<PHPCONFIG>) {
    	if (/^\s*\$HELPDESK_CONFIG\[/i) {
		s/\s*;.*$//;
		 # Now split the conf line up.
		 @hdconfs = split /\s*=\s*/;
		 # First part is the name
...
sub insert_message {
	...
	#open(MYOUTFILE, ">zzzz.out");
	#print MYOUTFILE "This is for test only\n ";
	#close(MYOUTFILE);
    ...
    # get requestor's email only
    if($header{'From'} =~ /.+<.+/)
    {	@authorInfo=split(/</, $header{'From'});
	$authorName=$authorInfo[0];
	$authorEmail=$authorInfo[1];
    	$authorEmail=~s/>//;
    }
    else{
    	$authorName=$unknown_contactname;
	$authorEmail=$header{'From'};
    }
    
    # connect to database
    $dbh = DBI->connect("DBI:mysql:$config{'dbname'}:$config{'dbhost'}", $config{'dbuser'}, $config{'dbpass'});
    
    # get company and requestor info
    $info_query="SELECT contact_first_name,contact_last_name, contact_company, contact_id  FROM contacts WHERE contact_email='$authorEmail' or contact_email2='$authorEmail'";
   #print $info_query;
   $sth = $dbh->prepare($info_query);
   $sth->execute()|| die ("Query failed");
   $match=0;
   if($sth->rows==0){
   	$note="The requestor's email:".$authorEmail." is not found in the contacts database!";
	$requestor=$authorName;
	$companyid=$unknown_compid;
	$requestortype=0;
	$requestorid=$unknown_contactid;
  }
   elsif($sth->rows>1){
   	$note="The requestor's email:".$authorEmail."  matches more than one records in contacts!";
	$requestor=$authorName;
	$companyid=$unknown_compid;
	$requestortype=0;
	$requestorid=$unknown_contactid;
	...
	$note ="NA";
	$requestor=$res->{contact_last_name}.", ".$res->{contact_first_name};
	$companyid=$res->{contact_company};
	$contactid=$res->{contact_id};
	$match=1;
   }
    $sth->finish();
   if($match) 
   {	$user_query="SELECT user_id FROM users WHERE user_contact='$contactid'";
   	$sth = $dbh->prepare($user_query);
	$sth->execute()|| die ("Query failed");
     	if($sth->rows==0){ #only a contact
		$requestortype=2;
		$requestorid=$contactid;
	}
	elsif($sth->rows==1){
		$requestortype=1; #system user
		$uid=$sth->fetchrow_hashref();
		$requestorid=$uid->{user_id};
	}
	else{	
		$requestortype=1;
		$requestorid=0;
		$note="The requestor's email matches more than one user records!";
    ...
    # quote all fields before insertion
    $db_parent = $dbh->quote($parent);
    $attachment = $dbh->quote($attachment);
    $authorEmail = $dbh->quote($authorEmail);
    $authorName = $dbh->quote($authorName);
   ...
   #$assignment = $dbh->quote($assignment);
    $note=$dbh->quote($note);
    $requestor=$dbh->quote($requestor);
    $companyid=$dbh->quote($companyid);
    $call_source=1;#for email
    ...
    $insert_query = "INSERT INTO " . $dbPrefix . "helpdesk_items (item_title, item_summary, item_source, item_requestor_email, item_requestor,item_requestor_id, item_requestor_type, item_company_id, item_created, item_updated)";
    $insert_query .= "VALUES ($subject,$body,$call_source,$authorEmail,$requestor, $requestorid, $requestortype, $companyid, NOW(),NOW())";
...
sub insert_attachment($) {
	...
	$sql_stmt = "INSERT into dotp_files (file_real_filename, file_name, file_type, file_size, file_date, file_description, file_task, file_version, file_version_id)  values (";
	$sql_stmt .= " '" . $fid . "',";
...
sub mail_report {
   ...
   # || $authorEmail eq $report_from_address ) {
   ...
   # }
	...
	#KZHAO  9-12-2006
	if ($parent) {
	    print MAIL "Subject: $HDconfig{'email_subject'}---Follow up of Ticket #$ticket\n";
	} else {
	    print MAIL "Subject: $HDconfig{'email_subject'}---Ticket #$ticket\n";
	}
	...
	#print MAIL "Thank your for registering yout request with us!\n\n";
	if ($parent) {
	  print MAIL "$HDconfig{'email_header'}--Followup to ticket #$ticket\n\n";
	} else {
	  print MAIL "$HDconfig{'email_header'}\n\n";
	}
	print MAIL "Ticket ID: $ticket\n";
	print MAIL "Requestor: $requestor\n";
	print MAIL "Subject  : $header{'Subject'}\n";
	#print MAIL "Lines    : $header{'Lines'}\n";
	#$newBody=$body;
        #$newBody=~s/\\n/<br>/g;	
	#print MAIL "Content  : $unquoted_body\n";
      	print MAIL "Note     : $note\n";
	print MAIL "View     : $app_root/index.php?=helpdesk&a=view&item_id=$ticket\n";
	print MAIL "Content  :\n$unquoted_body\n";
	...
	print MAIL "</style>\n";
	if ($parent) {
	    print MAIL "<title>$HDconfig{'email_header'}(Followup to ticket #$ticket)</title>\n";
	} else {
	    print MAIL "<title>$HDconfig{'email_header'}</title>\n";
	}
	print MAIL "</head>\n";
	print MAIL "<body>\n";
	print MAIL "\n";
	...
	print MAIL "	<TR>\n";
	print MAIL "		<TD nowrap><span class=title>$HDconfig{'email_header'}</span></td>\n";
	...
	print MAIL "	</tr>\n";
	print MAIL "	<TR>\n";
	print MAIL "		<TD bgcolor=white class=td>Requestor:</TD>\n";
	print MAIL "		<TD bgcolor=white class=td>$requestor</TD>\n";
	print MAIL "	</tr>\n";
	print MAIL "	<TR>\n";
	print MAIL "		<TD bgcolor=white class=td>Note:</TD>\n";
	print MAIL "		<TD bgcolor=white class=td>$note</TD>\n";
	print MAIL "	</tr>\n";
	print MAIL "	<TR>\n";
	...
	print MAIL "	</tr>\n";
	$htmlBody=$unquoted_body;
        $htmlBody=~s/\\n/<br>/g;	
	print MAIL "    <TR>\n";
        print MAIL "            <TD bgcolor=white class=td>Content:</TD>\n";
        print MAIL "            <TD bgcolor=white><font face=arial,san-serif size=2>$htmlBody</font></TD>";
        print MAIL "    </tr>\n";
	print MAIL "	<TR>\n";
	print MAIL "		<TD bgcolor=white nowrap class=td>View:</TD>\n";
	print MAIL "		<TD bgcolor=white nowrap class=td><a href=$app_root/index.php?m=helpdesk&a=view&item_id=$ticket>$app_root/index.php?=helpdesk&a=view&item_id=$ticket</a></TD>\n";
	print MAIL "	</tr>\n";
	print MAIL "</TABLE>\n";
	print MAIL "</body>\n";
	print MAIL "</html>\n";
...
sub mail_acknowledgement {
    ...
    if ( $authorEmail eq $report_to_address || $authorEmail eq $report_from_address) {
	...
	open(MAIL, "|$mailprog -t");
    	...
    	#ack='test2@localhost.localdomain';
	print MAIL "To: $authorEmail\n";
	print MAIL "From: $report_from_address\n";
	if ($parent)
	   {
	     #print MAIL "Subject: [#$ticket] Response to Ticket $ticket received\n";
	     print MAIL "Subject: $HDconfig{'email_subject'}\n";
	   }
	else
	    {
	...
	if ($parent)
	   ...
	   print MAIL "Thank you for registering your request with us!\n\n";
	   }
	 else
	     {
	...
	print MAIL "		<TD bgcolor=white class=td>$subject</TD>\n";
	print MAIL "	</tr>\n";
	$htmlBody2=$unquoted_body;
        $htmlBody2=~s/\\n/<br>/g;	
	print MAIL "	<TR>\n";
	print MAIL "		<TD bgcolor=white class=td>Content:</TD>\n";
	print MAIL "		<TD bgcolor=white class=td>$htmlBody2</TD>\n";
	print MAIL "	</tr>\n";
	print MAIL "	<TR>\n";
