<?php
//$ourCompanyId=$HELPDESK_CONFIG['the_company'];
// KZHAO: 8-8-2006
	...
	if($companyid!=0 && $companyid!=$HELPDESK_CONFIG['the_company']){
		//KZHAO   8-16-2006
                $sql=$sql."AND (contact_company=$companyid or contact_company=".
						$HELPDESK_CONFIG['the_company']." or contact_company in(". implode(",",array_keys(getAllowedCompanies()))."))";
		//$sql=$sql."AND (contact_company=$companyid or contact_company=$ourCompanyId)";
	}
...
function getAllowedProjectsForJavascript($activeonly=0){
  ...
  // select only active projects--KZHAO 2/8/2007
  if($activeonly)
      $whereclause.=" and project_status!=7";
      
  $sql = "SELECT project_id, project_name, company_name, company_id
          FROM " . $dbPrefix . "projects
          LEFT JOIN " . $dbPrefix . "companies ON company_id = " . $dbPrefix . "projects.project_company
          WHERE (". $whereclause
