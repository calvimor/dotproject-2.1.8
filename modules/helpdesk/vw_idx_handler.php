...
function vw_idx_handler ($type) {
  ...
  switch($type){
  	case 0:// newly created open ticket today
		...
		//  $where .= "(TO_DAYS(NOW()) - TO_DAYS(hi.item_updated) = 0) AND item_status = 1 AND (his.status_code = 0)";
  ...
  $sql = "SELECT hi.*,
          ...
          LEFT JOIN " . $dbPrefix . "helpdesk_item_status his ON his.status_item_id = hi.item_id
